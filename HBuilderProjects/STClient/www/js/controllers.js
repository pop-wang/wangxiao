...
app.controller('STController', function($scope,MapManager,$ionicModal,HTTPManager,Show,userInfo,$state,$rootScope,resize,dateTool) {
...
app.controller("MessageController",function ($scope,$stateParams,HTTPManager,userInfo,$interval) {
  ...
  HTTPManager.get(HOST+GET_USER_INFO,{userID:$stateParams.userID})
    ...
    function recMessage() {
      HTTPManager.post(HOST+RECEIVE_SIGLE_CHAT,{fromID:userInfo.getInfo("userid"),toID:$stateParams.userID}).then(function (result) {
        console.log(result);
